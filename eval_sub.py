...
def eval_sub(args):
    ...
    print(f"Attribute classes: {len(dataset.attr_names)}")
    ...
    # Track per-bird and per-attribute accuracies
    bird_correct = torch.zeros(len(dataset.bird_names))
    bird_total = torch.zeros(len(dataset.bird_names))
    attr_correct = torch.zeros(len(dataset.attr_names))
    attr_total = torch.zeros(len(dataset.attr_names))
...
if __name__ == "__main__":
    ...
    if not hasattr(args, 'dataset') or args.dataset != 'sub':
        print("Note: Running SUB evaluation (set dataset: sub in config for explicit selection)")
    ...
    # Run evaluation
    class_acc_meter, bird_acc, attr_acc, dataset = eval_sub(args)
    ...
    print("(Accuracy when bird has this substituted attribute)")
    for i, name in enumerate(dataset.attr_names):
